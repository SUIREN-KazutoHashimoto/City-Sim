...
export class World {
  ...
  populate(count: number): void {
    ...
    const pickHome = (x: number, z: number): number => { for (let t = 0; t < 12; t++) { const c = poiReg.findBest(POICategory.Home, x + (this.rng() - 0.5) * 400, z + (this.rng() - 0.5) * 400, this.rng()); if (c < 0) continue; const cnt = homeCount.get(c) ?? 0; if (cnt < poiReg.get(c).capacity) { homeCount.set(c, cnt + 1); return c; } } return -1; };
    const pickWork = (x: number, z: number, wealth: number, farRnd: boolean): number => { const bx = farRnd ? this.rng() * this.city.sizeMeters : x, bz = farRnd ? this.rng() * this.city.sizeMeters : z; for (let t = 0; t < 12; t++) { const c = poiReg.findBest(POICategory.Work, bx + (this.rng() - 0.5) * 300, bz + (this.rng() - 0.5) * 300, wealth); if (c < 0) continue; const cnt = workCount.get(c) ?? 0; if (cnt < poiReg.get(c).capacity) { workCount.set(c, cnt + 1); return c; } } return -1; };
    for (let n = 0; n < count; n++) {
      ...
      if (wp >= 0 && !willOwnCar) { const wpp = poiReg.get(wp); if (Math.hypot(wpp.x - home.x, wpp.z - home.z) > 700) { workCount.set(wp, (workCount.get(wp) ?? 1) - 1); wp = -1; } }
      s.workPOI[i] = wp;
      if (willOwnCar) { const lot = poiReg.findNearestFree(POICategory.Parking, home.x, home.z); if (lot >= 0 && poiReg.reserve(lot)) { const slot = this.city.takeSlot(lot); const px = slot >= 0 ? this.city.slotX(lot, slot) : poiReg.get(lot).x, pz = slot >= 0 ? this.city.slotZ(lot, slot) : poiReg.get(lot).z; const v = this.vehicles.create(i, lot, px, pz); if (v >= 0) { s.ownsCar[i] = 1; s.car[i] = v; this.vehicles.parkSlot[v] = slot; } else { poiReg.release(lot); if (slot >= 0) this.city.giveSlot(lot, slot); } } }
    }
  }
  step(dtSec: number): void {
    const s = this.store; const now = this.clock.totalSeconds;
    this.signals.update(dtSec); this.needs.update(s, dtSec);
    const budget = Math.min(s.count, 512);
    for (let k = 0; k < budget; k++) { const i = (this.decideCursor + k) % s.count; if (s.state[i] === AgentState.Idle && now >= s.nextDecideAt[i]) { this.brain.decide(s, i, this.clock); if (s.state[i] === AgentState.Idle) s.nextDecideAt[i] = now + 600 + this.rng() * 1200; } }
    ...
    this.bus.update(dtSec, (agent, stop) => this.onBusAlight(agent, stop), (stop, routeId, freeSeats) => this.collectBoarders(stop, routeId, freeSeats));
    this.bus.syncOnboard((agent, x, z) => { s.posX[agent] = x; s.posZ[agent] = z; });
    this.logistics.update(dtSec);
    this.buildTravelerIndex();
    for (let i = 0; i < s.count; i++) { const st = s.state[i]; if (st === AgentState.Traveling || st === AgentState.ToVehicle || st === AgentState.ToBusStop) this.walkStep(i, dtSec); }
    this.handleArrivedVehicles();
    for (let i = 0; i < s.count; i++) if (s.state[i] === AgentState.Engaged) this.activity(i, now, dtSec);
  }
  private collectBoarders(stop: { id: number }, routeId: number, freeSeats: number): number[] {
    const s = this.store; const out: number[] = [];
    for (let i = 0; i < s.count && out.length < freeSeats; i++) { if (s.state[i] !== AgentState.WaitingBus) continue; if (s.boardStop[i] !== stop.id || s.busRoute[i] !== routeId) continue; this.bus.setAlight(i, s.alightStop[i]); s.state[i] = AgentState.OnBus; s.waiting[i] = 0; out.push(i); }
    return out;
  }
  ...
  private computePedBlocks(): void {
    ...
    for (let v = 0; v < vs.count; v++) { if (vs.state[v] !== VehicleState.Driving || vs.speed[v] < 1) continue; const nt = net.nodes[vs.toNode[v]], nf = net.nodes[vs.fromNode[v]]; if (nt && (vs.posX[v] - nt.x) ** 2 + (vs.posZ[v] - nt.z) ** 2 < 100) this.vehBlock[vs.toNode[v]] = 1; if (nf && (vs.posX[v] - nf.x) ** 2 + (vs.posZ[v] - nf.z) ** 2 < 100) this.vehBlock[vs.fromNode[v]] = 1; }
  }
  ...
  private reserveGoal(i: number): boolean {
    ...
    if (this.storeUsable(g) && poi.reserve(g)) return true;
    const cat = poi.get(g).category;
    if (cat === POICategory.Food || cat === POICategory.Retail || cat === POICategory.Leisure) { const alt = poi.findBest(cat, s.posX[i], s.posZ[i], s.wealth[i]); if (alt >= 0 && this.storeUsable(alt) && poi.reserve(alt)) { s.goalPOI[i] = alt; const p = poi.get(alt); s.goalX[i] = p.x; s.goalZ[i] = p.z; return true; } }
    return false;
  }
  private beginTrip(i: number): void {
    const s = this.store;
    if (!this.reserveGoal(i)) { s.goalPOI[i] = -1; s.state[i] = AgentState.Idle; s.nextDecideAt[i] = this.clock.totalSeconds + 120 + this.rng() * 300; return; }
    ...
    if (s.pathHandle[i] <= 0 && tripDist > 300) { this.city.poi.release(s.goalPOI[i]); s.goalPOI[i] = -1; s.state[i] = AgentState.Idle; s.nextDecideAt[i] = this.clock.totalSeconds + 300 + this.rng() * 600; return; }
  ...
  private buildTravelerIndex(): void { this.grid.clear(); const s = this.store; for (let i = 0; i < s.count; i++) { const st = s.state[i]; if (st === AgentState.Traveling || st === AgentState.ToVehicle || st === AgentState.ToBusStop) this.grid.insert(i, s.posX[i], s.posZ[i]); } }
  private walkStep(i: number, dt: number): void {
    ...
    if (!hasPath && s.state[i] === AgentState.Traveling) { const dg = Math.hypot(finalX - s.posX[i], finalZ - s.posZ[i]); if (dg > 300) { if (s.goalPOI[i] >= 0) this.city.poi.release(s.goalPOI[i]); s.goalPOI[i] = -1; s.state[i] = AgentState.Idle; s.waiting[i] = 0; s.nextDecideAt[i] = this.clock.totalSeconds + 300 + this.rng() * 600; return; } }
    let tx = finalX, tz = finalZ; let arriving = !hasPath;
    if (hasPath) {
      const cur = s.pathCursor[i];
      if (cur < path.length) {
        const node = sw.nodes[path[cur]]; tx = node.x; tz = node.z;
        if (cur + 1 < path.length) { const e = sw.edgeBetween(path[cur], path[cur + 1]); if (e && e.crossing) { const nearCurb = Math.hypot(s.posX[i] - node.x, s.posZ[i] - node.z) < 3; const signalized = this.signals.modeOf(node.roadNode) !== null; const redPed = signalized && !this.signals.pedWalk(node.roadNode, e.axis); const carInside = this.vehBlock && this.vehBlock[node.roadNode] === 1; if (nearCurb && (redPed || carInside)) { s.waiting[i] = 1; s.velX[i] = 0; s.velZ[i] = 0; return; } } }
        s.waiting[i] = 0;
    ...
    if (arriving && d2 < 9) {
      ...
      if (s.state[i] === AgentState.ToBusStop) { s.velX[i] = 0; s.velZ[i] = 0; s.waiting[i] = 0; s.pathHandle[i] = -1; this.walkPaths[i] = EMPTY_PATH; s.state[i] = AgentState.WaitingBus; return; }
      s.state[i] = AgentState.Engaged; s.velX[i] = 0; s.velZ[i] = 0; s.waiting[i] = 0; s.pathHandle[i] = -1; this.walkPaths[i] = EMPTY_PATH;
      const p = this.city.poi.get(s.goalPOI[i]); if (p) s.dwellUntil[i] = this.computeDwellUntil(p.category); return;
    }
  ...
  private activity(i: number, now: number, dt: number): void {
    const s = this.store; const poi = this.city.poi.get(s.goalPOI[i]); if (!poi) { s.state[i] = AgentState.Idle; return; }
    ...
    if (now >= s.dwellUntil[i] || critical) { if (poi.maxStock > 0 && poi.stock > 0) poi.stock -= 1; this.city.poi.release(s.goalPOI[i]); s.goalPOI[i] = -1; s.state[i] = AgentState.Idle; s.nextDecideAt[i] = now; }
  }
  stats() {
    ...
    return { agents: this.store.count, buildings: this.city.buildings.length, nodes: this.city.net.nodes.length, pois: this.city.poi.size, vehiclesDriving: driving, vehiclesTotal: this.vehicles.count - this.bus.busCount - this.logistics.truckCount, parkingLots: this.city.parkingLots.length, signals: this.signals.count, buses: this.bus.busCount, busStops: this.bus.stops.length, busRoutes: this.bus.routes.length, trucks: this.logistics.truckCount, gates: this.city.gateNodes.length, stores, storesEmpty: empty };
  }
  activitySnapshot() {
    ...
    for (let i = 0; i < s.count; i++) {
      ...
      if (st === AgentState.Engaged) { const g = s.goalPOI[i]; const cat = g >= 0 ? this.city.poi.get(g).category : -1; if (cat === POICategory.Home) home++; else if (cat === POICategory.Work) work++; else if (cat === POICategory.Food) food++; else if (cat === POICategory.Leisure || cat === POICategory.Retail) leisure++; else idle++; continue; }
      idle++;
    }
