 ...
 *
 * Station geometry is generated from the same smoothed running path used by trains:
 * - platforms are always outside the train clearance envelope;
 * - passing loops follow the actual curved main line and merge with eased switches;
 * - platforms are short curved ribbons rather than one long box that can cut across a bend.
 */
export class RailRenderer {
  static readonly TRACK_Y = 8.2;
  private static readonly TRAIN_LENGTH = 25;
  private static readonly TRAIN_WIDTH = 2.9;
  private static readonly ACCEL = 0.78;
  private static readonly BRAKE = 1.18;
  /** Track-centre spacing between the through track and a passing-loop track. */
  private static readonly SIDING_OFFSET = 4.25;
  private static readonly PLATFORM_CLEARANCE = 0.42;
  private static readonly SWITCH_APPROACH = 24;
  ...
  private canEnterBlock(run: TrainRun, fromIndex: number, toIndex: number): boolean {
    ...
    for (const other of this.trainRuns) {
      ...
      if (other.currentStationIndex === toIndex) {
        if (!targetHasLoop || other.direction === run.direction) return false;
      }
    }
    return true;
  }
  ...
  private updateTrainMeshes(): void {
    ...
    for (let i = 0; i < this.trainRuns.length; i++) {
      ...
      this.pose(this.trainBody, i, run.x, RailRenderer.TRACK_Y + 1.85, run.z, heading, RailRenderer.TRAIN_LENGTH, 3.15, RailRenderer.TRAIN_WIDTH);
  ...
  /** Train lateral displacement uses exactly the same profile as the rendered passing-loop track. */
  private trainSidingOffset(run: TrainRun, smooth: SmoothLine): number {
    ...
    for (let i = 0; i < smooth.stationDistances.length; i++) {
      ...
      const half = this.platformLength(stationId) / 2;
      const profile = this.loopProfile(Math.abs(run.distance - smooth.stationDistances[i]), half);
      bestProfile = Math.max(bestProfile, profile);
    }
  ...
  private buildStations(): void {
    ...
    // A shared interchange may have multiple rail lines. Build each line's platform against its own track geometry.
    for (const line of this.rail.lines) {
      ...
      for (let stationIndex = 0; stationIndex < line.stationIds.length; stationIndex++) {
        ...
        if (passing) this.buildPassingLoop(smooth, centerDistance, length, loopBallast, loopRails);

        const platformWidth = passing ? 3.8 : (station.kind === RailStationKind.Central || station.kind === RailStationKind.SubCenter ? 4.8 : 4.2);
        const trainHalf = RailRenderer.TRAIN_WIDTH / 2;
        if (passing) {
          const platformOffset = RailRenderer.SIDING_OFFSET + trainHalf + RailRenderer.PLATFORM_CLEARANCE + platformWidth / 2;
          this.buildPlatformRibbon(smooth, centerDistance, length, platformOffset, platformWidth, true, stationId, platforms, roofs, signs, columns);
          this.buildPlatformRibbon(smooth, centerDistance, length, -platformOffset, platformWidth, false, stationId, platforms, roofs, signs, columns);
        } else {
          const side = ((stationId + line.id) & 1) === 0 ? 1 : -1;
          const platformOffset = side * (trainHalf + RailRenderer.PLATFORM_CLEARANCE + platformWidth / 2);
          this.buildPlatformRibbon(smooth, centerDistance, length, platformOffset, platformWidth, true, stationId, platforms, roofs, signs, columns);
        }
      }
    }
  ...
  /** Passing loop follows the actual smoothed main line, including curves. */
  private buildPassingLoop(smooth: SmoothLine, centerDistance: number, platformLength: number, ballast: StaticPart[], rails: StaticPart[]): void {
    ...
    for (const side of [-1, 1]) {
      ...
      for (let s = start; s <= end + 0.01; s = Math.min(end, s + step)) {
  ...
  /** 1 on the platform track, 0 at the switch tips, with smooth transitions. */
  private loopProfile(distanceFromStation: number, platformHalf: number): number {
    const fullUntil = platformHalf + 4;
  ...
  private buildPlatformRibbon(
    ...
    for (let s = start; s < end - 0.01; s += segmentLength) {
      const e = Math.min(end, s + segmentLength);
      const a = this.offsetPoint(smooth, s, lateralOffset), b = this.offsetPoint(smooth, e, lateralOffset);
      if (!a || !b) continue;
      this.pushRibbonSegment(a, b, RailRenderer.TRACK_Y + 0.38, 0.38, width, platforms);
      const mid = (s + e) * 0.5;
      if (Math.abs(mid - centerDistance) <= length * 0.36) {
        this.pushRibbonSegment(a, b, RailRenderer.TRACK_Y + 3.25, 0.18, width * 0.78, roofs);
      }
    }

    const columnSpacing = 16;
    for (let s = start + 6; s <= end - 5; s += columnSpacing) {
    ...
    if (includeSign) {
      const p = this.offsetPoint(smooth, centerDistance, lateralOffset); if (!p) return;
  ...
  private pushRibbonSegment(
    ...
    const mx = (a.x + b.x) / 2, mz = (a.z + b.z) / 2, angle = -Math.atan2(dz, dx);
    parts.push({ matrix: this.matrix(mx, y, mz, len + 0.18, height, width, angle) });
  }
  ...
  private platformLength(stationId: number): number {
    const station = this.rail.stations[stationId]; if (!station) return 50;
    if (station.kind === RailStationKind.Central) return 72;
    if (station.kind === RailStationKind.SubCenter) return 62;
    if (station.kind === RailStationKind.Terminal) return 56;
    return 50;
  }
  ...
  /** Convert the road-grid polyline into quadratic corner fillets. Station points may move slightly onto the curve. */
  private makeSmoothLine(line: RailLine): SmoothLine {
    ...
    for (let i = 1; i < src.length - 1; i++) {
      ...
      // Keep the curve inside the road-intersection envelope rather than cutting deeply across blocks.
      const radius = Math.min(22, la * 0.28, lb * 0.28);
      if (radius < 2.5) { push(p); continue; }
    ...
    for (let i = 1; i < path.length; i++) {
      length += Math.hypot(path[i].x - path[i - 1].x, path[i].z - path[i - 1].z); cumulative[i] = length;
    }
  ...
  private sampleSmooth(smooth: SmoothLine, distance: number): { x: number; z: number; heading: number } | null {
    ...
    const p = this.sampleSmoothPosition(smooth, d); if (!p) return null;
    const look = 2.5;
    const a = this.sampleSmoothPosition(smooth, Math.max(0, d - look));
    const b = this.sampleSmoothPosition(smooth, Math.min(smooth.length, d + look));
