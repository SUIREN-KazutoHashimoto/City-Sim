...
export class InstancedRenderer {
  ...
  private readonly categoryColor: Record<number, THREE.Color> = {
    ...
    [POICategory.Food]: new THREE.Color(0xc98b5a), [POICategory.Retail]: new THREE.Color(0xb9a06a),
  ...
  private buildGround(): void {
    const g = new THREE.Mesh(new THREE.PlaneGeometry(1, 1), new THREE.MeshStandardMaterial({ color: 0x1c3a24, roughness: 1 }));
    g.rotation.x = -Math.PI / 2; g.scale.set(1e5, 1e5, 1); g.position.set(0, -0.05, 0); g.receiveShadow = true; this.scene.add(g);
  }
  private buildRoads(net: RoadNetwork): void {
    ...
    for (const e of net.edges) {
      ...
      const mx = (a.x + b.x) / 2, mz = (a.z + b.z) / 2, angle = Math.atan2(b.z - a.z, b.x - a.x);
      const m = new THREE.Matrix4();
      m.compose(new THREE.Vector3(mx, 0.05, mz), new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -angle, 0)), new THREE.Vector3(e.length, 0.1, roadWidth(e.lanes)));
      boxes.push(m);
    }
    ...
    // 中央線(黄色い破線風の細い帯)を対向分離の視認用に引く
    this.buildCenterLines(net);
  }
  private buildCenterLines(net: RoadNetwork): void {
    ...
    for (const e of net.edges) {
      ...
      const mx = (a.x + b.x) / 2, mz = (a.z + b.z) / 2, angle = Math.atan2(b.z - a.z, b.x - a.x);
      const m = new THREE.Matrix4();
      m.compose(new THREE.Vector3(mx, 0.11, mz), new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -angle, 0)), new THREE.Vector3(e.length - 12, 0.05, 0.3));
      boxes.push(m);
    }
  ...
  private buildSidewalks(sw: SidewalkNetwork): void {
    ...
    for (const e of sw.edges) {
      ...
      const mx = (a.x + b.x) / 2, mz = (a.z + b.z) / 2, angle = Math.atan2(b.z - a.z, b.x - a.x);
      const m = new THREE.Matrix4();
      m.compose(new THREE.Vector3(mx, 0.09, mz), new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -angle, 0)), new THREE.Vector3(e.length + 3, 0.12, 3.0));
      boxes.push(m);
    }
  ...
  private buildBuildings(buildings: Building[]): void {
    ...
    for (let i = 0; i < buildings.length; i++) {
      const b = buildings[i];
      this.dummy.position.set(b.x, 0, b.z); this.dummy.scale.set(b.width, b.floors * fh, b.depth); this.dummy.rotation.set(0, 0, 0); this.dummy.updateMatrix();
      mesh.setMatrixAt(i, this.dummy.matrix); mesh.setColorAt(i, this.categoryColor[b.category] ?? new THREE.Color(0x9aa5b1));
    }
    mesh.instanceMatrix.needsUpdate = true; if (mesh.instanceColor) mesh.instanceColor.needsUpdate = true;
    mesh.castShadow = true; mesh.receiveShadow = true; this.scene.add(mesh); this.buildingMesh = mesh;
  }
  ...
  buildVehicles(capacity: number): void {
    ...
    mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage); mesh.frustumCulled = false; mesh.castShadow = true; this.scene.add(mesh); this.vehicleMesh = mesh;
  }
  buildSignals(net: RoadNetwork, signals: SignalSystem): void {
    ...
    vMesh.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(count * 3), 3); vMesh.frustumCulled = false;
    const pMesh = new THREE.InstancedMesh(new THREE.BoxGeometry(0.9, 0.9, 0.4), new THREE.MeshBasicMaterial(), count);
    pMesh.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(count * 3), 3); pMesh.frustumCulled = false;
    for (let k = 0; k < this.signalRefs.length; k++) {
      const { node, axis } = this.signalRefs[k]; const n = net.nodes[node];
      const ax = axis === 0 ? 5 : 0, az = axis === 1 ? 5 : 0;
      this.dummy.position.set(n.x + ax, 6.5, n.z + az); this.dummy.scale.setScalar(1); this.dummy.rotation.set(0, 0, 0); this.dummy.updateMatrix(); vMesh.setMatrixAt(k, this.dummy.matrix);
      const px = axis === 0 ? 0 : 5, pz = axis === 1 ? 0 : 5;
      this.dummy.position.set(n.x + px, 2.6, n.z + pz); this.dummy.updateMatrix(); pMesh.setMatrixAt(k, this.dummy.matrix);
    }
    vMesh.instanceMatrix.needsUpdate = true; pMesh.instanceMatrix.needsUpdate = true;
    this.scene.add(vMesh); this.scene.add(pMesh); this.vehSignalMesh = vMesh; this.pedSignalMesh = pMesh;
  }
  buildCrosswalks(net: RoadNetwork, signals: SignalSystem): void {
    ...
    const addBand = (cx: number, cz: number, dirX: number, dirZ: number, roadW: number) => {
      ...
      const px = -dz, pz = dx, angle = Math.atan2(dz, dx);
      const half = roadW / 2 - 0.3, pitch = SW + GAP, n = Math.max(1, Math.floor((roadW - 0.6) / pitch)), start = -((n - 1) * pitch) / 2;
      for (let k = 0; k < n; k++) {
        const off = start + k * pitch; if (Math.abs(off) > half) continue;
        const m = new THREE.Matrix4();
        m.compose(new THREE.Vector3(cx + px * off, y, cz + pz * off), new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -angle, 0)), new THREE.Vector3(CROSSWALK_DEPTH, 0.06, SW));
        stripes.push(m);
      }
    };
    for (const nodeId of signals.nodeIds) {
      const node = net.nodes[nodeId]; const scramble = signals.modeOf(nodeId) === SignalMode.Scramble;
      for (const edgeId of node.edges) {
        const e = net.edges[edgeId], nb = net.nodes[e.to];
        let dxx = nb.x - node.x, dzz = nb.z - node.z; const L = Math.hypot(dxx, dzz) || 1; dxx /= L; dzz /= L;
        const roadW = roadWidth(e.lanes), setback = crosswalkSetback(roadW);
  ...
  buildStopLines(net: RoadNetwork, signals: SignalSystem): void {
    ...
    for (const nodeId of signals.nodeIds) {
      ...
      for (const edgeId of node.edges) {
        const e = net.edges[edgeId], nb = net.nodes[e.to];
        let dxx = nb.x - node.x, dzz = nb.z - node.z; const L = Math.hypot(dxx, dzz) || 1; dxx /= L; dzz /= L;
        const roadW = roadWidth(e.lanes), setback = crosswalkSetback(roadW) + CROSSWALK_DEPTH * 0.5 + 0.8, angle = Math.atan2(dzz, dxx);
        // 停止線は進入側(右半分)のみ
        const px = -dzz, pz = dxx, halfShift = roadW / 4;
        const m = new THREE.Matrix4();
        m.compose(new THREE.Vector3(node.x + dxx * setback + px * halfShift, 0.14, node.z + dzz * setback + pz * halfShift),
          new THREE.Quaternion().setFromEuler(new THREE.Euler(0, -angle, 0)), new THREE.Vector3(0.6, 0.06, roadW / 2 - 0.4));
        bars.push(m);
      }
    }
  ...
  syncAgents(store: AgentStore): void {
    ...
    for (let i = 0; i < store.count; i++) {
      ...
      this.dummy.position.set(store.posX[i], 0, store.posZ[i]); this.dummy.rotation.set(0, -store.heading[i], 0); this.dummy.scale.setScalar(1); this.dummy.updateMatrix();
  ...
  syncVehicles(vs: VehicleStore): void {
    ...
    for (let v = 0; v < vs.count; v++) {
      ...
      this.dummy.position.set(vs.posX[v], 0, vs.posZ[v]); this.dummy.rotation.set(0, -vs.heading[v], 0); this.dummy.scale.setScalar(1); this.dummy.updateMatrix();
      mesh.setMatrixAt(n, this.dummy.matrix); mesh.setColorAt(n, this.carColors[vs.colorIdx[v] % this.carColors.length]); this.vehicleMap[n] = v; n++;
    }
  ...
  syncSignals(signals: SignalSystem): void {
    const vMesh = this.vehSignalMesh, pMesh = this.pedSignalMesh; if (!vMesh || !pMesh) return;
    for (let k = 0; k < this.signalRefs.length; k++) {
      const { node, axis } = this.signalRefs[k];
      const vc = signals.vehicleColor(node, axis); vMesh.setColorAt(k, vc === 'green' ? this.colGreen : vc === 'yellow' ? this.colYellow : this.colRed);
      const pc = signals.pedColor(node, axis); pMesh.setColorAt(k, pc === 'walk' ? this.colWalk : this.colDont);
    }
    if (vMesh.instanceColor) vMesh.instanceColor.needsUpdate = true; if (pMesh.instanceColor) pMesh.instanceColor.needsUpdate = true;
  }
}
