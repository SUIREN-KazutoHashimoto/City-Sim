...
export class FirstPersonController {
  ...
  update(dt: number): void {
    if (this.followTarget) {
      const t = this.followTarget, d = this.followDistance, cp = Math.cos(this.pitch);
      this.camera.position.set(t.x - Math.sin(this.yaw) * cp * d, t.y + Math.sin(this.pitch) * d + 1.2, t.z - Math.cos(this.yaw) * cp * d);
      this.camera.lookAt(t.x, t.y + 1.0, t.z); return;
    }
    this.forward.set(0, 0, -1).applyQuaternion(this.camera.quaternion);
    this.right.set(1, 0, 0).applyQuaternion(this.camera.quaternion);
    let mx = 0, mz = 0, my = 0;
    if (this.keys.has('KeyW')) mz += 1; if (this.keys.has('KeyS')) mz -= 1;
    if (this.keys.has('KeyD')) mx += 1; if (this.keys.has('KeyA')) mx -= 1;
    ...
    pos.addScaledVector(this.forward, mz * speed); pos.addScaledVector(this.right, mx * speed); pos.y += my * speed;
