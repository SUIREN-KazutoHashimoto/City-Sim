...
/**
 * Inspector: 左ボタン非押下でホバー調査。歩行者/車両/建物。
 * 中クリックで「歩行者 or 車両」を追跡(カメラ追従 + 常時ステータス)。
 */
export class Inspector {
  ...
  // 追跡中の対象(どちらか一方)
  private followKind: 'none' | 'agent' | 'vehicle' = 'none';
  private followId = -1;
  ...
  constructor(private world: World, private gfx: InstancedRenderer, private camera: THREE.PerspectiveCamera, private dom: HTMLElement) {
    this.el = this.panel('none');
    this.pinEl = this.panel('none');
    this.pinEl.style.left = '8px'; this.pinEl.style.bottom = '8px'; this.pinEl.style.borderColor = '#5a7fb0';
    this.dom.addEventListener('mousemove', (e) => {
      this.pointer.x = (e.clientX / window.innerWidth) * 2 - 1;
      this.pointer.y = -(e.clientY / window.innerHeight) * 2 + 1;
      this.el.style.left = `${e.clientX + 16}px`; this.el.style.top = `${e.clientY + 16}px`; this.hasPointer = true;
    });
  ...
  private panel(display: string): HTMLDivElement {
    ...
    el.style.cssText = ['position:fixed', 'z-index:20', 'pointer-events:none', 'font:12px/1.5 ui-monospace,monospace',
      'color:#dfe8f5', 'background:rgba(12,17,25,.92)', 'border:1px solid #3a4a63', 'border-radius:8px',
      'padding:8px 10px', 'max-width:290px', 'box-shadow:0 6px 20px rgba(0,0,0,.4)', 'white-space:pre-line', `display:${display}`].join(';');
    document.body.appendChild(el); return el;
  }
  /** 中クリック: ホバー中の対象(車両優先、なければ歩行者)を追跡トグル。 */
  ...
  getFollowPosition(): THREE.Vector3 | null {
    if (this.followKind === 'agent') {
      ...
      this.followPos.set(this.world.store.posX[this.followId], 0, this.world.store.posZ[this.followId]);
  ...
  update(): void {
    // 追跡ステータス(固定パネル)
    if (this.followKind === 'agent' && this.followId < this.world.store.count) {
      this.pinEl.textContent = '📌 追跡: 市民\n' + this.describeAgent(this.followId); this.pinEl.style.display = 'block';
    } else if (this.followKind === 'vehicle' && this.followId < this.world.vehicles.count) {
      this.pinEl.textContent = '📌 追跡: 車両\n' + this.describeVehicle(this.followId); this.pinEl.style.display = 'block';
    ...
    const aD = aHit.length ? aHit[0].distance : Infinity;
    const vD = vHit.length ? vHit[0].distance : Infinity;
    const bD = bHit.length ? bHit[0].distance : Infinity;
    if (aD <= vD && aD <= bD && aHit[0]?.instanceId != null) {
      const idx = this.gfx.agentIndexOf(aHit[0].instanceId);
      if (idx >= 0) { this.hoveredAgent = idx; this.el.textContent = this.describeAgent(idx); this.el.style.display = 'block'; return; }
    }
    if (vD <= bD && vHit[0]?.instanceId != null) {
      const vi = this.gfx.vehicleIndexOf(vHit[0].instanceId);
      if (vi >= 0) { this.hoveredVehicle = vi; this.el.textContent = this.describeVehicle(vi); this.el.style.display = 'block'; return; }
    }
    if (bHit.length && bHit[0].instanceId != null) { this.el.textContent = this.describeBuilding(bHit[0].instanceId); this.el.style.display = 'block'; }
    else this.hide();
  }
  ...
  private describeAgent(i: number): string {
    ...
    const goal = s.goalPOI[i]; const goalCat = goal >= 0 ? POICategory[this.world.city.poi.get(goal).category] : '—';
  ...
  private describeVehicle(v: number): string {
    ...
    const kmh = vs.speed[v] * 3.6, maxKmh = vs.maxSpeed[v] * 3.6, accel = vs.accel[v], driver = vs.driver[v];
    if (parked) return `🚗 車両 #${v}\n🅿️ 駐車中\n所有者 市民#${driver}\n車長 ${vs.length[v].toFixed(1)} m`;
    let goalCat = '—';
    if (driver >= 0) { const g = this.world.store.goalPOI[driver]; if (g >= 0) goalCat = POICategory[this.world.city.poi.get(g).category]; }
    const path = vs.paths[v]; const prog = path.length > 1 ? `${vs.pathCursor[v]}/${path.length - 1}` : '—';
    const status = vs.speed[v] < 0.3 ? '🛑 停止(信号待ち/渋滞)' : '🚗 走行中';
    return `🚗 車両 #${v}\n${status}\n速度 ${kmh.toFixed(0)} km/h (上限 ${maxKmh.toFixed(0)})\n` +
      `加速度 ${accel >= 0 ? '+' : ''}${accel.toFixed(1)} m/s²\n目的地 ${goalCat}   経路 ${prog}\n運転者 市民#${driver}`;
  }
  private describeBuilding(i: number): string {
    const b = this.world.city.buildings[i]; if (!b) return '';
    const cat = POICategory[b.category] ?? '?';
    const pois = this.world.city.poi.poisInBuilding(b.id);
    let occ = 0, cap = 0; for (const p of pois) { occ += p.occupancy; cap += p.capacity; }
    const uses = pois.length ? pois.map((p) => `  · ${POICategory[p.category]} (在 ${p.occupancy}/${p.capacity})`).join('\n') : '  · 用途なし';
