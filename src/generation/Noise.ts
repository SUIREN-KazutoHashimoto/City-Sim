...
export class ValueNoise2D {
  private perm: Uint16Array; private grad: Float32Array; private readonly size = 256; private readonly mask = 255;
  constructor(seed: number) {
    const rng = makeRng(seed); this.grad = new Float32Array(this.size);
    for (let i = 0; i < this.size; i++) this.grad[i] = rng() * 2 - 1;
    this.perm = new Uint16Array(this.size * 2); const p = new Uint16Array(this.size);
  ...
  noise(x: number, z: number): number {
    const x0 = Math.floor(x), z0 = Math.floor(z); const fx = x - x0, fz = z - z0;
    const u = fx * fx * (3 - 2 * fx), v = fz * fz * (3 - 2 * fz);
    const n00 = this.valueAt(x0, z0), n10 = this.valueAt(x0 + 1, z0), n01 = this.valueAt(x0, z0 + 1), n11 = this.valueAt(x0 + 1, z0 + 1);
