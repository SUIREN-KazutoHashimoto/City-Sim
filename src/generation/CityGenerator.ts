...
export interface ParkingLot { id: number; poiId: number; x: number; z: number; width: number; depth: number; capacity: number; }
...
export class CityGenerator {
  ...
  generate(): void {
    ...
    const connect = (a: number, b: number, u: number) => {
      if (a < 0 || b < 0) return;
      const cls = u >= this.urbanThreshold + 0.25 ? RoadClass.Arterial : u >= this.urbanThreshold ? RoadClass.Local : RoadClass.Arterial;
  ...
  private addParking(x: number, z: number, size: number): void {
    ...
    this.parkingLots.push({ id: this.parkingLots.length, poiId, x, z, width: size, depth: size, capacity });
  }
  ...
  private registerPOIs(buildingId: number, x: number, z: number, cat: POICategory, floors: number): void {
    ...
    if (cat === POICategory.Home && this.rng() < 0.15) this.poi.add({ category: POICategory.Food, x, z, priceTier, capacity: 20, buildingId });
    if ((cat === POICategory.Work || cat === POICategory.Leisure) && this.rng() < 0.3) this.poi.add({ category: POICategory.Retail, x, z, priceTier, capacity: 30, buildingId });
  }
}
