...
export interface CityPlanningOptions {
  subCenters: number;
 ...
 * 同じseedならCBD/副都心/工業・物流拠点と地価場は決定論的に再生成できる。
 */
export class CityPlanning {
  ...
  sample(x: number, z: number): PlanningSample {
    ...
    const centerInfluence = Math.max(cbdInf, subInf);

    // 市街化判定用。中心・副都心を核にしつつnoiseで連続した市街地を作る。
    const urbanScore = clamp(
      0.33 * n + 0.18 * detail + centerInfluence * 0.72 + indInf * 0.34 + logInf * 0.18 - edgePenalty * 0.18,
      0,
      1,
    );
    ...
    // 地価は市街化度とは別。中心アクセスを強くし、工業/物流は抑える。
    const amenityNoise = this.detailNoise.fbm(x / 900 + 11.3, z / 900 - 6.7, 3);
    const landValue = clamp(
      0.18 + cbdInf * 0.62 + subInf * 0.40 + urbanScore * 0.26 + amenityNoise * 0.12 - indInf * 0.28 - logInf * 0.20,
      0.05,
      1,
    );
    ...
    const density = clamp(districtDensity * (0.58 + landValue * 0.58), 0.04, 1);
    return { district, urbanScore, landValue, density, centerInfluence };
  }
