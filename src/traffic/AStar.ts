...
export class AStar {
  ...
  constructor(private net: PathGraph, private mode: 'drive' | 'walk' = 'drive') {
    ...
    this.cameFrom = new Int32Array(n); this.closed = new Uint8Array(n); this.openFlag = new Uint8Array(n);
  }
  findPath(start: number, goal: number): number[] {
    const net = this.net;
    this.g.fill(Infinity); this.f.fill(Infinity); this.closed.fill(0); this.openFlag.fill(0); this.cameFrom.fill(-1);
    this.heap.length = 0;
    ...
    while (this.heap.length) {
      ...
      for (const edgeId of net.nodes[cur].edges) {
        ...
        if (tentative < this.g[nb]) {
          this.cameFrom[nb] = cur; this.g[nb] = tentative;
  ...
  private push(node: number): void {
    ...
    while (i > 0) { const p = (i - 1) >> 1; if (this.f[this.heap[p]] <= this.f[this.heap[i]]) break; [this.heap[p], this.heap[i]] = [this.heap[i], this.heap[p]]; i = p; }
  }
  private pop(): number {
