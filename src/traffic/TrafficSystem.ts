...
export class TrafficSystem {
  ...
  /** 駐車中の車を start→goal で走らせる。成功で true。 */
      ...
      for (let k = 0; k < occ.length; k++) {
        ...
        if (!isTerminal) {
          ...
          if (!this.signals.vehicleGreen(vs.toNode[v], axis)) {
            ...
            if (toStopLine > 0.5) gapStop = toStopLine; // ジレンマゾーンは通過
          }
        }
