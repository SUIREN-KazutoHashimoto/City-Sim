...
/** IDM + 信号(停止線) + 前方車追従 + 車線オフセット(右側通行)による車両交通。 */
export class TrafficSystem {
  ...
  dispatch(vehicle: number, sx: number, sz: number, gx: number, gz: number): boolean {
    ...
    vs.pathCursor[vehicle] = 1; vs.parkPOI[vehicle] = -1; vs.speed[vehicle] = 0;
  ...
  private findEdge(from: number, to: number): number {
    for (const eid of this.net.nodes[from].edges) if (this.net.edges[eid].to === to) return eid;
    return -1;
  }
  update(dt: number): void {
    ...
    for (let e = 0; e < this.net.edges.length; e++) {
      const occ = this.net.edges[e].occupants;
      ...
      for (let k = 0; k < occ.length; k++) {
        ...
        if (isTerminal && remaining < 4) {
          vs.state[v] = VehicleState.Arrived; vs.speed[v] = 0; vs.segT[v] = 1;
          this.updateWorldPos(v); continue;
        }
  ...
  private idm(v: number, gap: number, leadSpeed: number): number {
    const vs = this.vs;
    const sp = vs.speed[v], v0 = vs.maxSpeed[v], a = vs.aMax[v], b = vs.bComf[v];
  ...
  private advanceEdge(v: number): void {
    const vs = this.vs;
    const path = vs.paths[v], next = vs.pathCursor[v] + 1;
    if (next >= path.length) { vs.state[v] = VehicleState.Arrived; vs.speed[v] = 0; vs.segT[v] = 1; return; }
    const from = path[vs.pathCursor[v]], to = path[next];
    vs.pathCursor[v] = next; const carry = vs.speed[v];
    this.enterEdge(v, from, to); vs.speed[v] = carry;
  }
  /** segT→ワールド座標。進行方向の右手に車線オフセットを掛け、対向車と重ならせない。 */
  private updateWorldPos(v: number): void {
    const vs = this.vs;
    const nf = this.net.nodes[vs.fromNode[v]], nt = this.net.nodes[vs.toNode[v]];
    if (!nf || !nt) return;
    const t = vs.segT[v];
    let dx = nt.x - nf.x, dz = nt.z - nf.z; const L = Math.hypot(dx, dz) || 1; dx /= L; dz /= L;
    // 右手方向(進行方向を+90度回転)。右側通行なので自分の車線側へずらす。
    const rx = dz, rz = -dx;
    const edge = vs.edge[v] >= 0 ? this.net.edges[vs.edge[v]] : null;
    const off = laneOffset(edge ? edge.lanes : 1);
    const cx = nf.x + (nt.x - nf.x) * t, cz = nf.z + (nt.z - nf.z) * t;
    vs.posX[v] = cx + rx * off;
    vs.posZ[v] = cz + rz * off;
    vs.heading[v] = Math.atan2(dz, dx);
  }
  private rebuildOccupants(): void {
    ...
    for (let v = 0; v < vs.count; v++) {
      if (vs.state[v] !== VehicleState.Driving) continue;
      const e = vs.edge[v]; if (e >= 0) edges[e].occupants.push(v);
    }
