...
export class SignalSystem {
  ...
  constructor(net: RoadNetwork, seed = 20240521, scrambleFraction = 0.15) {
    ...
    for (const n of net.nodes)
      if (n.hasSignal && n.edges.length >= 3) { this.signalOf[n.id] = this.nodeIds.length; this.nodeIds.push(n.id); }
  ...
  update(dt: number): void {
    for (let k = 0; k < this.nodeIds.length; k++) {
      const prog = this.programs[this.mode[k]];
      this.timer[k] += dt;
      if (this.timer[k] >= prog[this.step[k]].duration) { this.timer[k] = 0; this.step[k] = (this.step[k] + 1) % prog.length; }
    }
  }
  ...
  vehicleColor(node: number, axis: 0 | 1): 'green' | 'yellow' | 'red' | null {
    ...
    if (st.vehGreenAxis === axis) return st.vehYellow ? 'yellow' : 'green'; return 'red';
  }
