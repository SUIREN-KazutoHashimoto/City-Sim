...
export interface RoadEdge {
  id: number; from: number; to: number; length: number; lanes: number;
  speedLimit: number; roadClass: RoadClass; occupants: number[];
}
const SPEED_BY_CLASS: Record<RoadClass, number> = {
  [RoadClass.Highway]: 27, [RoadClass.Arterial]: 16, [RoadClass.Local]: 11, [RoadClass.Path]: 1.4,
};
...
/** 進行方向に対する車線中心の横オフセット(m)。右側通行。片側 lanes 車線の中央。 */
